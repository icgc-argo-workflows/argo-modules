...
workflow_full_name = {
    ...
    'dna-seq-alignment': 'DNA Alignment'
}
...
def get_files_info(file_to_upload,pipeline_info):
    return {
        ...
        'fileAccess': 'controlled', # should this be set controlled always?
...
def main(args):
    ...
    # convert software version into string
    ...
    aligned_file = [ f for f in args.files_to_upload if (f.endswith('.bam') or f.endswith('.cram')) ][0] # this variable is not used anywhere else
    ...
    for f in args.files_to_upload:
        ...
        with open("%s.alignment.payload.json" % str(uuid.uuid4()), 'w') as f:
