...
analysis_type,study_id,patient,sex,status,sample,lane,fastq_1,fastq_2,read_group,single_end,read_group_count,analysis_json
    ...
    def validate_and_transform(self, row):
        ...
        tmp_dict={
            ...
            "bai_crai" : row[self._bai_crai_col],
    ...
    def _validate_bam_cram(self, row):
        ...
        row[self._bam_cram_col]=os.path.abspath(row[self._bam_cram_col])

    def _validate_bai_crai(self, row):
        """Assert that expected crai is correct."""
        if not row.get(self._bai_crai_col):
            os.mkdir("out")
            sym_dest="out/"+row[self._bam_cram_col].split("/")[-1]
            os.symlink(row[self._bam_cram_col],sym_dest)

            row[self._bam_cram_col]=os.path.abspath(sym_dest)

            if row[self._bam_cram_col].endswith(".bam"):
                pysam.index(
                    os.path.abspath(sym_dest),
                    os.path.abspath(sym_dest+".bai")
                    )
                row[self._bai_crai_col]=os.path.abspath(sym_dest+".bai")
            else:
                pysam.index(
                    os.path.abspath(sym_dest),
                    os.path.abspath(sym_dest+".crai")
                    )
                row[self._bai_crai_col]=os.path.abspath(sym_dest+".crai")
        else:
            row[self._bai_crai_col]=os.path.abspath(row[self._bai_crai_col])
